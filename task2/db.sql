...
select  e.name as employee, d.name as department, count(s.amount) as count from employees e join departments d on e.department_id=d.id left join salaries
s on e.id =s.employee_id group by  e.name ;